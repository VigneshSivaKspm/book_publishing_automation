...
function Sep() {
  return (
    <div
      className="w-px h-8 mx-0.5 self-center flex-shrink-0"
...
export default function BookEditor({
  ...
  useEffect(() => {
    const handleSelectionChange = () => {
      ...
      if (!selection || selection.rangeCount === 0 || selection.isCollapsed)
        return;
      const canvasEl =
        document.getElementById("active-page-canvas") ||
        document.querySelector(".page-canvas.active");
      if (!canvasEl) return;
  ...
  useEffect(() => {
    const handleCopy = (e: ClipboardEvent) => {
      ...
      if (
        ...
        if (
          activeTa.selectionStart !== null &&
          activeTa.selectionEnd !== null
        ) {
          const selectedText = activeTa.value.substring(
            activeTa.selectionStart,
            activeTa.selectionEnd,
          );
          if (selectedText) {
  ...
  const cycleMcqAnswer = (blockId: string) => {
    ...
    commit(
      ...
      `Updated Q${targetBlock.text.match(/^(\d+)/)?.[1] || ""} answer to (${nextAns})`,
    );
  };
  ...
  const handleAiSolveAnswers = async () => {
    ...
    try {
      ...
      commit(
        { ...book, pages: nextPages },
        `AI solved ${solvedCount} question answers`,
      );
    } catch {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && (e.key === "a" || e.key === "A")) {
        ...
        if (!isInput) {
          ...
          e.preventDefault();
          const bodyNode =
            document.querySelector("#page-body-content") ||
  ...
  const pushHistory = useCallback(
    (snapshot: BookDocument, mode: "push" | "replace") => {
  ...
  const updatePage = (
    pageId: string,
    updater: (p: BookPage) => BookPage,
    opts?: { typing?: boolean; msg?: string },
  ) => {
    commit(
      {
        ...bookRef.current,
        pages: bookRef.current.pages.map((p) =>
    ...
    updatePage(
      ...
      (p) => {
        ...
        const idx = selectedBlockId
          ? next.findIndex((b) => b.id === selectedBlockId)
          : next.length - 1;
  ...
  const addBlock = (type: ContentBlock["type"]) => {
    if (type === "mcq") {
      addBlocks(
        [emptyMcqTemplate(nextMcqNumber(activePage?.blocks || []))],
        "MCQ added",
      );
      return;
    }
  ...
  const runAlign = async () => {
    ...
    try {
      ...
      const { page: alignedPage } = autoCorrectPage(
        activePage,
        startMcqNum,
        isSyllabus ? "document" : "qa",
      );

      if (isSyllabus) {
        const nextPages = book.pages.map((p) =>
          p.id === activePage.id ? alignedPage : p,
        );
        const { book: reflowedBook, blocksShifted } = reflowBookOverflow({
          ...book,
          pages: nextPages,
        });
        commit(
          reflowedBook,
          `✓ Pages auto-aligned & filled without bottom free space`,
        );
        showToast(
          blocksShifted > 0
            ? `✨ Pages packed to fill completely with zero bottom free space`
            : `✨ Pages already fully packed!`,
        );
      } else {
        const { updatedBlocks, solvedCount } = await aiSolveUnansweredMcqs(
          alignedPage.blocks,
        );
  ...
  const setBatchFontSize = (newSize: number, targetBlockId?: string) => {
    ...
    const nextPages = book.pages.map((p) => {
      ...
      return {
        ...p,
        blocks: p.blocks.map((b) =>
          ids.includes(b.id) ? { ...b, fontSize: newSize } : b,
        ),
      };
    });
    commit(
      { ...book, pages: nextPages },
      `Updated font size to ${newSize}pt for ${ids.length} block(s)`,
    );
  };
  ...
  const adjustSelectedFontSize = (delta: number, targetBlockId?: string) => {
    ...
    commit(
      { ...book, pages: nextPages },
      `Adjusted font size for ${ids.length} block(s)`,
    );
  };

  const changeBlockCase = (
    ...
    if (targetCase === "upper") {
      ...
      newText = target.text.replace(/\b[a-zA-Z]/g, (char) =>
        char.toUpperCase(),
      );
    }
  ...
  const fitContentToPage = (pageId?: string) => {
    ...
    if (canvasEl) {
      ...
      if (scrollH < clientH * 0.82) {
        ...
        const nextPages = book.pages.map((p) =>
          p.id === targetPage.id ? { ...p, blocks: updatedBlocks } : p,
        );
        commit(
          { ...book, pages: nextPages },
          `✨ Auto-fit vertical page layout for Page ${targetPage.number}`,
        );
        showToast(
          `✨ Scaled line spacing and font size to fill Page ${targetPage.number}`,
        );
        return;
      }
    }
    ...
    commit(
      reflowedBook,
      blocksShifted > 0
        ? `Reflowed ${blocksShifted} block(s) across pages`
        : `Page layout optimized`,
    );
    showToast(
      blocksShifted > 0
        ? `✨ Shifted ${blocksShifted} overflowing block(s) to next page`
        : `✨ Page layout already optimal!`,
    );
  };
  ...
  const runCompile = () => {
    ...
    window.setTimeout(() => {
      ...
      next.pages.forEach((p, i) => {
        ...
        if (hasH && paras >= 2 && !hasImg)
          found.push(`Page ${i + 1}: add an image after the heading`);
      });
  ...
  const handleFile = async (
    ...
    if (mode === "image") {
      ...
      setTips((t) =>
        [
          `Image on page ${activePage.number} — keep it under the related heading`,
          ...t,
        ].slice(0, 5),
      );
      return;
    ...
    try {
      const result = await ocrImageToBlocks(
        ...
        (p) => {
          ...
          if (pct > 25)
            setOcrStatusText(
              isSyllabus
                ? "Extracting headings, lists, tables & formulas..."
                : "Extracting questions, Tamil text & math formulas...",
            );
        },
      ...
      if (
        !isSyllabus &&
        result.answerKey &&
        Object.keys(result.answerKey).length > 0
      ) {
        ...
        commit(
          { ...book, pages: nextPages },
          `Applied ${applied} answers from the scanned answer key`,
        );
        return;
      }
      ...
      if (cm?.title) {
        ...
        if (
          !book.headerFooter.headerLeft ||
          book.headerFooter.headerLeft === book.title
        )
          nextHf.headerLeft = cm.title;
      }
      ...
      const finalBlocks = [
        ...activePage.blocks.filter((b) => b.text.trim() || b.imageUrl),
        ...outBlocks,
      ];
      commit(
        {
          ...
          pages: book.pages.map((p) =>
            p.id === activePage.id ? { ...p, blocks: finalBlocks } : p,
          ),
        },
        isSyllabus
    ...
    try {
      ...
      for (let i = 0; i < slideTexts.length; i++) {
        const stepPct = Math.min(
          94,
          85 + Math.round(((i + 1) / slideTexts.length) * 9),
        );
        setOcrPct(stepPct);
        setOcrStatusText(
          isSyllabus
        ...
        if (meta.number && !chapterMeta.number)
        ...
        if (
          ...
          Object.keys(structured.answerKey).length > 0
        ) {
      ...
      if (!isSyllabus) {
        ...
        for (let p = 0; p < contentPages.length; p++) {
          if (hasKey) {
            const { blocks, applied } = applyAnswerKeyToBlocks(
              contentPages[p],
              scannedKey,
            );
      ...
      const isFreshBook =
        book.pages.length === 1 &&
        activePage.blocks.every(
          (b) =>
            !b.text.trim() || b.type === "heading1" || b.type === "paragraph",
        );
      ...
      if (isSyllabus) {
        ...
        if (isFreshBook) {
          combinedBlocks = allScannedBlocks;
        } else {
          const prior = book.pages
            .slice(0, activeIndex)
            .flatMap((p) => p.blocks);
          const current = activePage.blocks.filter(
            (b) => b.text.trim() || b.imageUrl || b.type === "image",
          );
          const subsequent = book.pages
            .slice(activeIndex + 1)
            .flatMap((p) => p.blocks);
          combinedBlocks = [
            ...prior,
        ...
        renumbered = packBlocksIntoPages(
          combinedBlocks,
          book.paperSize,
          cols,
          1,
        ...
        basePages[activeIndex] = {
          ...activePage,
          blocks: firstBlocks.length ? firstBlocks : activePage.blocks,
        };

        nonEmpty.slice(1).forEach((blocks, i) => {
          basePages.splice(activeIndex + 1 + i, 0, {
            id: uid("pg"),
            number: 0,
            blocks,
          });
        });
        renumbered = basePages.map((p, i) => ({ ...p, number: i + 1 }));
      }
      ...
      if (chapterMeta.number)
        nextHeaderFooter.chapterNumber = chapterMeta.number;
      ...
      commit(
        {
          ...book,
          title:
            isFreshBook && chapterMeta.title ? chapterMeta.title : book.title,
      ...
      showToast(
        `Doc Scan: ${nonEmpty.length} page(s) from ${file.name}${chapterMeta.number ? ` — Chapter ${chapterMeta.number}` : ""}`,
      );
    } catch {
  ...
  const pastePaper = () => {
    if (book.bookMode === "questions-only") {
      ...
      if (!raw?.trim()) return;
      const newBlocks = structureDocumentText(raw);
      const cols = book.headerFooter.layoutColumns || 2;
      const isFreshBook =
        book.pages.length === 1 &&
        activePage.blocks.every(
      ...
      const baseBlocks = isFreshBook
        ? []
        : book.pages
            .flatMap((p) => p.blocks)
            .filter((b) => b.text.trim() || b.imageUrl || b.type === "image");
      const allBlocks = [...baseBlocks, ...newBlocks];
      const hasChapterBadge = !!(
      ...
      commit(
        { ...book, pages: packed },
        `Added ${newBlocks.length} syllabus blocks · Packed into ${packed.length} full pages`,
      );
      showToast(
        `✓ Packed syllabus into ${packed.length} full pages without bottom free space`,
      );
      return;
    }
    ...
    addBlocks(
      blocks,
      mcqCount ? `${mcqCount} Q · ${answered} answers` : "Text added",
    );
  };
  ...
  const makeAnswerKey = () => {
    ...
    commit(
      {
        ...book,
        pages: [...book.pages, page].map((p, i) => ({ ...p, number: i + 1 })),
      },
      "Answer key page added",
    );
    setActivePageId(page.id);
  };
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      const isModalOpen = Boolean(
        document.querySelector(
          ".fixed.z-\\[60\\], .fixed.z-\\[70\\], .fixed.z-\\[100\\]",
        ),
      );
      ...
      if (!meta && (e.key === "Backspace" || e.key === "Delete")) {
        const curPage = bookRef.current.pages.find(
          (p) => p.id === activePageId,
        );
        if (!curPage) return;

        if (isEditingText) {
          const targetInput = activeEl as
            | HTMLInputElement
            | HTMLTextAreaElement;
        ...
        if (
          ...
          selText.length > 0 ||
          savedSelectionBlocksRef.current.length > 0
        ) {
          e.preventDefault();
          ...
          updatePage(curPage.id, (p) => ({ ...p, blocks: finalBlocks }), {
  ...
  return (
    ...
    >
      {toast && (
        <div
          className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[70] px-4 py-2 rounded text-[13px] text-white shadow-lg"
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3.5 py-2 flex-wrap gap-2 bg-slate-100 border-b border-slate-300 shadow-xs">
              ...
              <button
                ...
                onClick={() =>
                  commit(
                    {
                      ...book,
                      headerFooter: { ...book.headerFooter, layoutColumns: 1 },
                    },
                    "1 Column Layout",
                  )
                }
            ...
            <div className="flex border border-slate-300 bg-white p-0.5">
              {(["A4", "B5", "8×8"] as PaperSize[]).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    commit({ ...book, paperSize: s }, `Size updated to ${s}`)
                  }
          ...
          <div className="flex items-center gap-1">
            {book.bookMode === "questions-only" ? (
              <>
                <Btn
                  onClick={insertTopic}
                  className="px-2.5 py-1 text-[11.5px] font-bold text-slate-800"
                  title="Insert Chapter / Topic"
                >
                  + Topic
                </Btn>
                <Btn
          ...
          {tips.length > 0 && (
            ...
            >
              ...
              >
                ...
                <button onClick={() => setTips([])} style={{ color: "#999" }}>
                  ×
                </button>
              </div>
              {tips.map((t) => (
                <div key={t} style={{ color: "#444" }}>
                  · {t}
                </div>
              ))}
            </div>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      columnRule:
                        ...
                        book.headerFooter.showColumnDivider !== false
                          ? "1.2px solid #000000"
                          : "none",
                    }}
                  >
                      ...
                      if (block.type === "image") {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              outline: isSel ? "2px dashed #0E7490" : undefined,
                              outlineOffset: "4px",
                            }}
                          >
                            {isSel && (
                              ...
                              >
                                <span className="font-bold text-teal-700 text-[10px] uppercase tracking-wider">
                                  🖼 Image
                                </span>
                                ...
                                >
                                  {[
                                    100, 140, 180, 220, 280, 340, 400, 480, 560,
                                  ].map((h) => (
                            ...
                            {block.imageUrl && (
                              <div
                                className={`relative inline-block max-w-full ${imgMarginClass}`}
                              >
                                <img
                                  src={block.imageUrl}
                          ...
                          {isSel ? (
                            <>
                              ...
                              <textarea
                                value={
                                  block.type === "mcq"
                                    ? stripInlineAnswerTags(block.text)
                                    : block.text
                                }
                                onChange={(e) => {
                                  if (block.type === "mcq") {
                                    ...
                                    const extracted =
                                      textVal.match(/\[✓\s*([A-E])\]/i)?.[1];
                                    const cleanVal =
                                      stripInlineAnswerTags(textVal);
                                    const patch: Partial<ContentBlock> = {
                                      text: cleanVal,
                                    };
                                    if (extracted)
                                      patch.answer = extracted.toUpperCase();
                                ...
                                rows={Math.max(
                                  block.type === "mcq" ? 4 : 1,
                                  Math.min(10, block.text.split("\n").length),
                                )}
                                className="w-full resize-none outline-none bg-transparent"
                                style={{
                                  fontSize: size,
                                  fontWeight: block.type.startsWith("heading")
                                    ? 700
                                    : 400,
                            ...
                            >
                              {block.text ? (
                                block.type === "table" ? (
                                  (() => {
                                    const { header, rows } = parseMarkdownTable(
                                      block.text,
                                    );
                                    if (header.length === 0)
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      return rawLines.map((line, i) => {
                                        const cleanLine =
                                          stripInlineAnswerTags(line);
                                        if (!cleanLine) return null;
                                        if (
                                          i > 0 &&
                                          !optStarted &&
                                          isMcqOptionLine(cleanLine)
                                        )
                                          optStarted = true;
                                ...
                                ) : renderTextWithMath(block.text).includes(
                                    '<span class="katex">',
                                  ) ? (
                                  <div
                                    style={{ lineHeight: 1.2 }}
                                    dangerouslySetInnerHTML={{
                                      __html: renderTextWithMath(block.text),
                                    }}
                                  />
                                ) : (
                                  block.text
                                ...
                                <span className="italic text-slate-400 font-sans text-[11px]">
                                  [Click to edit]
                                </span>
                              )}
                            </div>
      ...
      {/* Bottom Status Bar */}
      <div
        className="h-7 flex-shrink-0 flex items-center gap-3 px-3 text-[11px]"
        style={{ background: "#0E7490", color: "rgba(255,255,255,0.92)" }}
      >
        <button onClick={() => goPage(-1)} className="hover:underline">
          Prev
        </button>
        <span>
          Page {activePage?.number ?? 0} / {book.pages.length}
        </span>
        <button onClick={() => goPage(1)} className="hover:underline">
          Next
        </button>
        ...
        <button
          onClick={() => setZoom((z) => Math.min(1.15, z + 0.08))}
          className="hover:underline"
        >
          +
        </button>
      ...
      <HeaderFooterModal
        ...
        onChange={(hf) =>
          commit({ ...book, headerFooter: hf }, "Header & Watermark updated")
        }
